...
<PackageManagerTabs>
  <PackageManagerContent value="npm">
    ```bash
    npx shadcn@latest add https://somaui.dev/[[STYLE]]/progress
    ```
  </PackageManagerContent>
  <PackageManagerContent value="pnpm">
    ```bash
    pnpm dlx shadcn@latest add https://somaui.dev/[[STYLE]]/progress
    ```

  </PackageManagerContent>
  <PackageManagerContent value="bun">
    ```bash
    bunx --bun shadcn@latest add https://somaui.dev/[[STYLE]]/progress
    ```
  </PackageManagerContent>
...
The installed file includes a reusable wrapper `SomaProgressBar`. This wrapper serves as an excellent starting point for use throughout your codebase.

<ComponentPreview name="progressbar-reusable" />

The `SomaProgressBar` component extends the props of React Aria Progress and adds:
